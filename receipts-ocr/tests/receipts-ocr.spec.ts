...
test.describe('Receipts OCR App', () => {
  ...
  test('should process image with OCR', async ({ page }) => {
    ...
    await expect(page.locator('.results h3')).toContainText('Extracted Data');
  });
  ...
  test('should show raw OCR text in details', async ({ page }) => {
    ...
    // Expand raw text details
    await page.locator('.raw-text summary').click();
    await expect(page.locator('.raw-text pre')).toBeVisible();
  });
});
...
test.describe('Backend Integration', () => {
  test('backend health endpoint responds', async ({ request }) => {
    ...
    if (response.ok()) {
      ...
      expect(data.service).toBe('paddleocr');
    }
  });
